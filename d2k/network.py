...
class Network:
    ...
    def read_darknet_weights(self, weights_blob):
        ...
        in_dim = self.config[0][1]['channels']
        out_dim = dict()
        weights = dict()
        ...
        for (i, (section, options)) in enumerate(self.config[1:]):
            if section == '[convolutional]':
                ...
                if batch_normalize:
                    ...
                    l_weights = [r.read_float32(shape) for shape in [(filters,), (filters,), (filters,), (filters,)]]
                    l_weights = _order(l_weights, [1, 0, 2, 3])

                    weights[f'bn_{i}'] = l_weights
                else:
                    bias = r.read_float32((filters,))

                # Darknet saves (bias) (out_dim, in_dim, height, width)
                # TF needs it          (height,  width,  in_dim, out_dim) (bias)

                l_weights = r.read_float32((filters, in_dim, size, size))
                l_weights = [l_weights.transpose([2, 3, 1, 0])]

                if not batch_normalize:
                    l_weights.append(bias)

                weights[f'conv_{i}'] = l_weights
                ...
                assert section in ['[shortcut]', '[upsample]', '[maxpool]', '[yolo]'] # no weights, same in_dim
